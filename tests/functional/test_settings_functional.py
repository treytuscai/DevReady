...
def login(client):
    ...
    return client.post("/login", data={"username": "testuser", "password": "password123"}, follow_redirects=True)

def test_change_email_success(client, app, sample_data):
    login(client)
    response = client.post("/newEmail", data={"email": "newemail@example.com"}, follow_redirects=True)
